...
from .text_loaders import read_text_file
...
def rel_meta(root: str, full_path: str) -> dict:
    ...
    meta = {"original_path": rel, "filename": fn, "ext": ext, "document_type": ext.lstrip(".")}
    for i in range(8):
...
def main():
    ...
    files.sort()
    log(LOG_PATH, f"FOUND txt/md files={len(files)}")
    ...
    def handle_one(abs_path: str):
        try:
            ...
            text = (read_text_file(abs_path) or "").strip()
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as ex:
        ...
        for fut in tqdm(as_completed(futs), total=len(futs), desc="TXT/MD files"):
