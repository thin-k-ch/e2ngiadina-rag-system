...
class Agent:
    ...
    async def llm(self, messages, temperature: float = 0.2, stream: bool = False):

        """Backward-compatible LLM entrypoint.

        - stream=False: returns full text (awaitable)

        - stream=True : returns an async generator of text chunks (iterable via `async for`)
        ...
        if stream:
            ...
            async def _gen():
                ...
                async for piece in self.llm_stream(messages, temperature=temperature):

                    yield piece

            return _gen()

        return await self.llm_text(messages, temperature=temperature)
