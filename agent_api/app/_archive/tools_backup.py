...
class Tools:
    ...
    def search_hybrid(self, queries: list[str], top_k_each: int = 8, max_total: int = 36, size: int | None = None, **kwargs):
        print(f"🔍 HYBRID SEARCH: {len(queries)} queries")
        # Accept OpenAI-style alias 'size' (total desired hits) and ignore extra kwargs
        if size is not None:
            try:
                max_total = int(size)
            except Exception:
        ...
        for h in chroma_hits + es_hits:
            # Normalize hit to ensure it's a dict
            if isinstance(h, str):
                h = {"path": h, "text": "", "score": None, "id": h}
            hid=h.get("id")
            if not hid or hid in seen:
                continue
            seen.add(hid)
